...
(function () {
  ...
  function parseOrderMessage(text) {
    ...
    const remaining = [];
    lines.forEach(line => {
      // Ignorar líneas de encabezado/instrucciones
      ...
      for (const key in labels) {
        ...
        if (m) {
          ...
          if (key === 'phone') { 
            res.phone = val.replace(/[^\d+]/g, '');
          } else if (key === 'payment') {
            res.payment = val;
          } else if (key === 'amount') {
            res.amount = val.replace(/[^\d.,]/g, '');
          } else if (!res[key]) {
            res[key] = val;
          }
          matched = true; break;
    ...
    // Buscar dirección en texto libre
  ...
  function applyParsedOrder(text) {
    ...
    // Método de pago: detectar si es transferencia
    if (p.payment) {
      const paySelect = formNewOrder.querySelector('[name="payment_method"]');
      if (paySelect) {
        const isTransfer = /(bancolombia|nequi|daviplata|transferencia|pse|banco|consignaci)/i.test(p.payment);
        paySelect.value = isTransfer ? 'card' : 'cash';
      }
    }
    // Monto
    if (p.amount) {
